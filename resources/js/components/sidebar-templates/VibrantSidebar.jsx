// resources/js/components/sidebar-templates/VibrantSidebar.jsx
...
export default function VibrantSidebar() {
  ...
  // Vibrant styling - bold, colorful, energetic
  const shell = "relative h-screen bg-gradient-to-br from-orange-50 via-pink-50 to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900";
  const card = `relative flex h-full flex-col ${widthCls} transition-[width] duration-300 overflow-hidden bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm`;
  
  const scrollAreaCls = "flex-1 overflow-y-auto min-h-0 py-3 px-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-orange-300 dark:[&::-webkit-scrollbar-thumb]:bg-slate-600";

  const itemBase = "group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-gray-700 dark:text-gray-300 hover:bg-white dark:hover:bg-slate-800 hover:shadow-lg transition-all duration-200";
  const itemActive = "bg-white dark:bg-slate-800 text-gray-900 dark:text-white font-bold shadow-lg scale-[1.02]";
      ...
      return (
        <div key={`sec-${section.name}-${index}`} className="flex items-center justify-center py-2 mx-2 mt-2">
          <div 
            ...
            style={{ background: `linear-gradient(135deg, ${config.baseColor}40, ${config.hoverColor}40)` }}
          >
    ...
    return (
      <div key={`sec-${section.name}-${index}`} className="px-3 py-2 mx-2 mt-3">
        <div 
          ...
          style={{ background: `linear-gradient(90deg, ${config.baseColor}20, transparent)` }}
        >
          <span style={{ color: config.baseColor }}>{React.cloneElement(section.icon, { className: "w-4 h-4" })}</span>
          <span className="text-xs font-bold uppercase tracking-wider" style={{ color: config.baseColor }}>{section.name}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <aside className={shell}>
      <div className={card}>
        ...
        <div className="sticky top-0 z-10 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 overflow-hidden">
              <div 
                className="h-12 w-12 rounded-2xl flex items-center justify-center shadow-lg overflow-hidden"
                style={{ 
                  background: `linear-gradient(135deg, ${getThemeColor(theme, 'primary', 'color')}, ${getThemeColor(theme, 'secondary', 'color')}, ${getThemeColor(theme, 'tertiary', 'color')})` 
                }}
              >
          ...
          {menu.map((item, i) => {
            ...
            return (
              <Link
                ...
                style={active ? { 
                  background: `linear-gradient(135deg, ${sectionConfig.lightColor}90, ${sectionConfig.lightColor}50)`,
                  border: `2px solid ${sectionConfig.baseColor}40`,
                } : {}}
              ...
              >
                ...
                {active && (
                  <div 
                    ...
                    style={{ background: `linear-gradient(135deg, ${sectionConfig.baseColor}, ${sectionConfig.hoverColor})` }}
                  />
                )}
              </Link>
            );
