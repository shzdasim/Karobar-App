// resources/js/components/sidebar-templates/AuroraSidebar.jsx
...
export default function AuroraSidebar() {
  ...
  // Aurora styling - flowing colors like northern lights
  const shell = "relative h-screen overflow-hidden";
  const card = "relative flex h-full flex-col w-64 overflow-hidden bg-slate-900/95";
  
  const scrollAreaCls = "flex-1 overflow-y-auto min-h-0 py-3 px-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-emerald-500/30";
  ...
  const itemActive = "text-white font-medium bg-gradient-to-r from-emerald-500/20 via-teal-500/20 to-cyan-500/20";

  const renderSectionHeader = (section, index) => {
    return (
      <div key={`sec-${section.name}-${index}`} className="px-3 py-2 mx-2 mt-4">
        <div className="flex items-center gap-2">
          <span className="text-emerald-400">{React.cloneElement(section.icon, { className: "w-4 h-4" })}</span>
          <span className="text-xs font-bold uppercase tracking-wider text-emerald-400/70">{section.name}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <aside className={shell}>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-1/2 -left-1/2 w-full h-full bg-gradient-to-br from-emerald-500/20 via-teal-500/10 to-cyan-500/20 blur-3xl animate-pulse" />
        <div className="absolute -bottom-1/2 -right-1/2 w-full h-full bg-gradient-to-tl from-cyan-500/20 via-blue-500/10 to-emerald-500/20 blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
      </div>
      
      <div className={card}>
        {/* Header with Logo */}
        <div className="sticky top-0 z-10 bg-slate-900/80 backdrop-blur-sm border-b border-emerald-500/20 px-4 py-4">
          <div className="flex items-center justify-center">
            <div className="relative">
              <div className="h-12 w-12 rounded-full flex items-center justify-center bg-gradient-to-br from-emerald-400 via-teal-500 to-cyan-500 shadow-lg shadow-emerald-500/30 overflow-hidden">
                <Logo className="h-10 w-10" alt={brandName} />
              </div>
            </div>
          </div>
          <div className="text-center mt-2">
            <span className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">{brandName}</span>
          </div>
        </div>
          ...
          {menu.map((item, i) => {
            ...
            return (
              ...
              >
                ...
                <span>{item.name}</span>
                {active && (
                  <div className="absolute right-2 w-2 h-2 rounded-full bg-gradient-to-r from-emerald-400 to-cyan-400 shadow-lg shadow-emerald-400/50" />
                )}
              </Link>
            );
          })}
