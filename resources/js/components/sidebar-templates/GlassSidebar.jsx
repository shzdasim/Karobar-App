// resources/js/components/sidebar-templates/GlassSidebar.jsx
...
export default function GlassSidebar() {
  ...
  const rawMenu = useMemo(() => [
    ...
    { name: "Quotations", path: "/quotations", icon: <ClipboardDocumentListIcon className="w-5 h-5" />, perm: "quotation.view", parent: "invoices" },
  ...
  // Glass styling - heavy glassmorphism effect
  const shell = "relative h-screen bg-gradient-to-br from-blue-50/50 via-purple-50/30 to-pink-50/50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900";
  const card = `relative flex h-full flex-col ${widthCls} transition-[width] duration-300 overflow-hidden bg-white/30 dark:bg-slate-900/40 backdrop-blur-2xl border-r border-white/40 dark:border-white/10 shadow-2xl`;
  
  const scrollAreaCls = "flex-1 overflow-y-auto min-h-0 py-3 px-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/50 dark:[&::-webkit-scrollbar-thumb]:bg-white/20";

  const itemBase = "group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-gray-700 dark:text-gray-300 hover:bg-white/50 dark:hover:bg-white/10 backdrop-blur-sm transition-all duration-200";
  const itemActive = "bg-white/70 dark:bg-white/20 backdrop-blur-md text-gray-900 dark:text-white font-medium shadow-lg border border-white/60 dark:border-white/20";
    ...
    if (collapsed) {
      return (
        <div key={`sec-${section.name}-${index}`} className="flex items-center justify-center py-2 mx-2 mt-2">
          <div 
            className="p-2 rounded-xl backdrop-blur-md bg-white/40 dark:bg-white/10 border border-white/50 dark:border-white/20"
          >
    ...
    return (
      <div key={`sec-${section.name}-${index}`} className="px-3 py-2 mx-2 mt-3">
        <div className="flex items-center gap-2 px-2">
          <div 
            className="p-1.5 rounded-lg backdrop-blur-sm bg-white/40 dark:bg-white/10 border border-white/40 dark:border-white/10"
          >
            <span style={{ color: config.baseColor }}>{React.cloneElement(section.icon, { className: "w-3.5 h-3.5" })}</span>
          </div>
          <span className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{section.name}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <aside className={shell}>
      <div className={card}>
        {/* Header */}
        <div className="sticky top-0 z-10 bg-white/40 dark:bg-white/5 backdrop-blur-xl border-b border-white/50 dark:border-white/10 px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 overflow-hidden">
              <div 
                className="h-10 w-10 rounded-xl flex items-center justify-center backdrop-blur-md bg-white/60 dark:bg-white/20 border border-white/60 dark:border-white/30 shadow-lg"
              >
                <span 
                  className="font-bold text-lg"
                  style={{ color: getThemeColor(theme, 'primary', 'color') }}
                >K</span>
              </div>
              {!collapsed && (
          ...
          {menu.map((item, i) => {
            ...
            return (
              <Link
                ...
                style={active ? { 
                  backgroundColor: sectionConfig.lightColor + '60',
                } : {}}
        ...
        {/* Footer */}
        <div className="sticky bottom-0 z-10 bg-white/40 dark:bg-white/5 backdrop-blur-xl border-t border-white/50 dark:border-white/10 px-3 py-3">
          <button
            onClick={() => setCollapsed((v) => !v)}
            className="w-full flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-white/50 dark:hover:bg-white/10 backdrop-blur-sm transition-all duration-200 border border-white/40 dark:border-white/10"
