// resources/js/components/sidebar-templates/GradientSidebar.jsx
...
export default function GradientSidebar() {
  ...
  // Gradient styling - vibrant gradient backgrounds
  const shell = `relative h-screen bg-gradient-to-b from-[${primaryColor}]20 to-[${secondaryColor}]20 dark:from-slate-900 dark:to-slate-800`;
  const card = `relative flex h-full flex-col ${widthCls} transition-[width] duration-300 overflow-hidden bg-gradient-to-b from-white/95 to-gray-50/95 dark:from-slate-800/95 dark:to-slate-900/95 backdrop-blur-sm`;
  
  const scrollAreaCls = "flex-1 overflow-y-auto min-h-0 py-3 px-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-thumb]:bg-slate-600";

  const itemBase = "group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-gray-700 dark:text-gray-300 hover:bg-white dark:hover:bg-slate-700 transition-all duration-200";
  const itemActive = "bg-gradient-to-r from-white to-gray-50 dark:from-slate-700 dark:to-slate-600 text-gray-900 dark:text-white font-medium shadow-md";
      ...
      return (
        <div key={`sec-${section.name}-${index}`} className="flex items-center justify-center py-2 mx-2 mt-2">
          <div 
            ...
            style={{ background: `linear-gradient(135deg, ${config.baseColor}30, ${config.hoverColor}30)` }}
          >
    ...
    return (
      <div key={`sec-${section.name}-${index}`} className="px-3 py-2 mx-2 mt-3">
        <div 
          ...
          style={{ background: `linear-gradient(90deg, ${config.baseColor}15, transparent)` }}
        >
          <span style={{ color: config.baseColor }}>{React.cloneElement(section.icon, { className: "w-4 h-4" })}</span>
  ...
  return (
    <aside 
      ...
      style={{ background: `linear-gradient(180deg, ${primaryColor}15, ${secondaryColor}10)` }}
    >
      <div className={card}>
          ...
          {menu.map((item, i) => {
            ...
            return (
              <Link
                ...
                style={active ? { 
                  background: `linear-gradient(90deg, ${sectionConfig.lightColor}80, ${sectionConfig.lightColor}40)`,
