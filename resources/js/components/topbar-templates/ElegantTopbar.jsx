// resources/js/components/topbar-templates/ElegantTopbar.jsx
...
export default function ElegantTopbar() {
  ...
  return (
    <>
      <div className="bg-gradient-to-r from-slate-50 via-white to-slate-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 border-b border-slate-200 dark:border-slate-700 w-full max-w-full overflow-hidden">
        <div className="flex items-center h-12 px-3 w-full min-w-0">
          {/* Elegant Logo */}
          <div className="flex items-center gap-2 mr-6 flex-shrink-0">
            <div className="h-9 w-9 rounded-full overflow-hidden shadow-xl ring-2 ring-slate-200 dark:ring-slate-600 flex-shrink-0">
              <Logo className="h-9 w-9" alt={brandName} />
            </div>
            <div className="flex flex-col">
              <span className="text-sm font-serif font-semibold text-slate-800 dark:text-slate-100 tracking-wide whitespace-nowrap">{brandName}</span>
              <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-widest whitespace-nowrap">Management</span>
            </div>
          </div>
            ...
            {getFilteredItems(flatMenu).map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-300 hover:bg-slate-100 dark:hover:bg-slate-800"
      ...
      {openDropdown && (() => {
        ...
        return (
          <div
            ref={dropdownRef}
            className="fixed bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-100 dark:border-slate-700 py-2 z-[99999]"
            style={{ top: dropdownPosition.top, left: dropdownPosition.left, minWidth: '180px' }}
          >
            <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-700/30">
              <div className="flex items-center gap-2">
                <div className="p-1 rounded-lg bg-white dark:bg-slate-700 shadow-sm" style={{ color: colors.base }}>
                  {React.cloneElement(section?.icon, { className: "w-4 h-4" })}
                </div>
                <span className="text-sm font-serif font-medium text-slate-700 dark:text-slate-200">{section?.name}</span>
              </div>
            </div>
              ...
              {filteredItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium transition-all duration-200 text-left rounded-lg font-serif"
