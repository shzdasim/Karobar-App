// resources/js/components/topbar-templates/GlassTopbar.jsx
...
export default function GlassTopbar() {
  ...
  return (
    <>
      <div className="bg-gradient-to-r from-gray-100/50 via-white/30 to-gray-100/50 dark:from-slate-900/50 dark:via-slate-800/30 dark:to-slate-900/50 backdrop-blur-2xl border-b border-white/40 dark:border-white/10 w-full max-w-full overflow-hidden">
        <div className="flex items-center h-12 px-3 w-full min-w-0">
          {/* Glass Logo */}
          <div className="flex items-center gap-2 mr-4 flex-shrink-0">
            <div className="h-8 w-8 rounded-xl overflow-hidden shadow-lg ring-1 ring-white/50 dark:ring-white/20 backdrop-blur-sm bg-white/30 dark:bg-slate-800/30 flex-shrink-0">
              <Logo className="h-8 w-8" alt={brandName} />
            </div>
            <span className="text-sm font-bold text-gray-900 dark:text-white drop-shadow-sm whitespace-nowrap">{brandName}</span>
          </div>
            ...
            {getFilteredItems(flatMenu).map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium transition-all duration-200 backdrop-blur-sm ${
                    active 
                      ? "bg-white/60 dark:bg-slate-700/60 shadow-lg ring-1 ring-white/50 dark:ring-white/20" 
                      : "hover:bg-white/40 dark:hover:bg-slate-800/40 text-gray-700 dark:text-gray-300"
                  }`}
      ...
      {openDropdown && (() => {
        ...
        return (
          <div
            ref={dropdownRef}
            className="fixed bg-white/80 dark:bg-slate-800/80 backdrop-blur-2xl rounded-2xl shadow-2xl ring-1 ring-white/60 dark:ring-white/20 py-2 z-[99999]"
            style={{ top: dropdownPosition.top, left: dropdownPosition.left, minWidth: '180px' }}
          >
            <div className="px-3 py-2 mx-2 rounded-xl mb-1 backdrop-blur-sm" style={{ backgroundColor: colors.light + '80' }}>
              ...
              {filteredItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium transition-all duration-150 text-left rounded-xl backdrop-blur-sm"
                    style={{ backgroundColor: itemActive ? colors.light + '80' : undefined, color: itemActive ? colors.base : undefined }}
                  >
