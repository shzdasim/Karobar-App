// resources/js/components/topbar-templates/VibrantTopbar.jsx
...
export default function VibrantTopbar() {
  ...
  return (
    <>
      <div className="bg-white dark:bg-slate-900 border-b-4 w-full max-w-full overflow-hidden" style={{ borderColor: primaryColor }}>
        <div className="flex items-center h-12 px-3 w-full min-w-0">
          {/* Vibrant Logo */}
          <div className="flex items-center gap-2 mr-4 flex-shrink-0">
            <div className="h-8 w-8 rounded-xl overflow-hidden shadow-lg flex-shrink-0" style={{ background: `linear-gradient(135deg, ${primaryColor}, ${secondaryColor})` }}>
              <Logo className="h-8 w-8" alt={brandName} />
            </div>
            <span className="text-base font-black text-gray-900 dark:text-white tracking-tight whitespace-nowrap">{brandName}</span>
          </div>
            ...
            {getFilteredItems(flatMenu).map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-bold transition-all duration-200 hover:bg-gray-100 dark:hover:bg-slate-800"
      ...
      {openDropdown && (() => {
        ...
        return (
          <div
            ref={dropdownRef}
            className="fixed bg-white dark:bg-slate-800 rounded-xl shadow-2xl py-2 z-[99999] overflow-hidden"
            style={{ top: dropdownPosition.top, left: dropdownPosition.left, minWidth: '180px', borderTop: `4px solid ${colors.base}` }}
          >
            <div className="px-3 py-2 mx-2 rounded-lg mb-1" style={{ backgroundColor: colors.light }}>
              <div className="flex items-center gap-2">
                <span style={{ color: colors.base }}>{React.cloneElement(section?.icon, { className: "w-4 h-4" })}</span>
                <span className="text-sm font-bold" style={{ color: colors.base }}>{section?.name}</span>
              </div>
            </div>
              ...
              {filteredItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm font-bold transition-all duration-150 text-left rounded-lg"
