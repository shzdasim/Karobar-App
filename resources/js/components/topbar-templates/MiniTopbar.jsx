// resources/js/components/topbar-templates/MiniTopbar.jsx
...
export default function MiniTopbar() {
  ...
  return (
    <>
      <div className="bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 shadow-sm w-full max-w-full overflow-hidden">
        <div className="flex items-center h-12 w-full min-w-0">
          {/* Compact Logo */}
          <div className="flex items-center px-3 border-r border-gray-200 dark:border-slate-700 flex-shrink-0">
          ...
          {/* Compact Navigation */}
          <nav className="flex-1 flex items-center gap-0.5 overflow-x-auto px-1 min-w-0" role="navigation">
            {getFilteredItems(flatMenu).map((item) => {
              ...
              return (
                <Link
                  ...
                  style={{ backgroundColor: active ? colors.light : undefined, color: active ? colors.base : undefined }}
                  title={item.name}
                >
              ...
              return (
                <div key={section.name} className="relative flex-shrink-0">
                  <button
                    ...
                    style={{ backgroundColor: isActiveSection ? colors.light : undefined, color: isActiveSection ? colors.base : undefined }}
                    title={section.name}
                  >
      ...
      {openDropdown && (() => {
        ...
        return (
          <div
            ref={dropdownRef}
            className="fixed bg-white dark:bg-slate-800 rounded-lg shadow-lg ring-1 ring-black/5 dark:ring-white/10 py-1 z-[99999]"
            style={{ top: dropdownPosition.top, left: dropdownPosition.left, minWidth: '180px' }}
          >
            <div className="px-3 py-1.5 border-b border-gray-100 dark:border-slate-700" style={{ backgroundColor: colors.light }}>
              ...
              {filteredItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-medium transition-all duration-150 text-left"
