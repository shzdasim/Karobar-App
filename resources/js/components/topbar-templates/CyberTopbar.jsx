// resources/js/components/topbar-templates/CyberTopbar.jsx
...
export default function CyberTopbar() {
  ...
  return (
    <>
      <div className="bg-slate-950 border-b border-cyan-500/30 shadow-lg shadow-cyan-500/10 w-full max-w-full overflow-hidden">
        <div className="flex items-center h-12 px-3 w-full min-w-0">
          {/* Cyber Logo */}
          <div className="flex items-center gap-2 mr-4 flex-shrink-0">
            <div className="h-8 w-8 rounded-lg overflow-hidden border-2 border-cyan-400 shadow-lg shadow-cyan-400/30 flex-shrink-0" style={{ backgroundColor: primaryColor + '20' }}>
              <Logo className="h-8 w-8" alt={brandName} />
            </div>
            <span className="text-sm font-bold text-cyan-400 tracking-wider uppercase whitespace-nowrap" style={{ textShadow: '0 0 10px ' + primaryColor }}>{brandName}</span>
          </div>
            ...
            {getFilteredItems(flatMenu).map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium transition-all duration-200 border border-transparent ${
                    active ? "border-cyan-400/50 bg-cyan-400/10 text-cyan-400" : "text-slate-300 hover:text-cyan-400 hover:border-cyan-400/30"
                  }`}
              ...
              return (
                <div key={section.name} className="relative">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium transition-all duration-200 border border-transparent ${
                      isActiveSection ? "border-cyan-400/50 bg-cyan-400/10 text-cyan-400" : "text-slate-300 hover:text-cyan-400 hover:border-cyan-400/30"
                    }`}
                    style={{ fontFamily: 'monospace' }}
                  >
      ...
      {openDropdown && (() => {
        ...
        return (
          <div
            ref={dropdownRef}
            className="fixed bg-slate-900 border border-cyan-500/30 py-1 z-[99999] shadow-lg shadow-cyan-500/20"
            style={{ top: dropdownPosition.top, left: dropdownPosition.left, minWidth: '180px' }}
          >
            <div className="px-3 py-2 border-b border-cyan-500/20 bg-cyan-400/10">
              <div className="flex items-center gap-2">
                <span className="text-cyan-400">{React.cloneElement(section?.icon, { className: "w-4 h-4" })}</span>
                <span className="text-xs font-semibold text-cyan-400 uppercase tracking-wider" style={{ fontFamily: 'monospace' }}>{section?.name}</span>
              </div>
            </div>
            <div className="py-1">
              {filteredItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 text-sm font-medium transition-all duration-150 text-left ${
                      itemActive ? "bg-cyan-400/20 text-cyan-400 border-l-2 border-cyan-400" : "text-slate-300 hover:text-cyan-400 hover:bg-cyan-400/10"
                    }`}
                    style={{ fontFamily: 'monospace' }}
                  >
