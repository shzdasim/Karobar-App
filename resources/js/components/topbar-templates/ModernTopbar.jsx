// resources/js/components/topbar-templates/ModernTopbar.jsx
...
export default function ModernTopbar() {
  ...
  return (
    <>
      <div className="bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-800 w-full max-w-full overflow-hidden">
        <div className="flex items-center h-14 px-4 w-full min-w-0">
          {/* Modern Logo */}
          <div className="flex items-center gap-2 mr-6 flex-shrink-0">
            <div className="h-9 w-9 rounded-xl overflow-hidden shadow-lg ring-2 ring-gray-100 dark:ring-slate-700 flex-shrink-0">
              <Logo className="h-9 w-9" alt={brandName} />
            </div>
            <span className="text-base font-bold text-gray-900 dark:text-white tracking-tight whitespace-nowrap">{brandName}</span>
          </div>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${
                    active ? "text-white shadow-md" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-300 dark:hover:text-white dark:hover:bg-slate-800"
                  }`}
                  style={{ backgroundColor: active ? colors.base : undefined }}
                >
              ...
              return (
                <div key={section.name} className="relative">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${
                      isActiveSection ? "text-white shadow-md" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-300 dark:hover:text-white dark:hover:bg-slate-800"
                    }`}
                    style={{ backgroundColor: isActiveSection ? colors.base : undefined }}
                  >
      ...
      {openDropdown && (() => {
        ...
        return (
          <div
            ref={dropdownRef}
            className="fixed bg-white dark:bg-slate-800 rounded-2xl shadow-2xl ring-1 ring-black/5 dark:ring-white/10 py-2 z-[99999]"
            style={{ top: dropdownPosition.top, left: dropdownPosition.left, minWidth: '200px' }}
          >
            <div className="px-3 py-2 border-b border-gray-100 dark:border-slate-700">
              <span className="text-xs font-bold uppercase tracking-wider" style={{ color: colors.base }}>{section?.name}</span>
            </div>
            <div className="py-1">
              {filteredItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium transition-all duration-150 text-left rounded-lg mx-1"
                    style={{ 
