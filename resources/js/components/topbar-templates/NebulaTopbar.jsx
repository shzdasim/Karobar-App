// resources/js/components/topbar-templates/NebulaTopbar.jsx
...
export default function NebulaTopbar() {
  ...
  const secondaryColor = getThemeColor(theme, 'secondary', 'color');
  const tertiaryColor = getThemeColor(theme, 'tertiary', 'color');

  return (
    <>
      <div className="relative bg-slate-950 border-b border-slate-800 overflow-hidden w-full max-w-full">
        {/* Nebula Background Effect */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-0 left-1/4 w-96 h-96 rounded-full opacity-20 blur-3xl" style={{ background: `radial-gradient(circle, ${primaryColor}, transparent)` }} />
          <div className="absolute bottom-0 right-1/4 w-80 h-80 rounded-full opacity-15 blur-3xl" style={{ background: `radial-gradient(circle, ${secondaryColor}, transparent)` }} />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full opacity-10 blur-3xl" style={{ background: `radial-gradient(circle, ${tertiaryColor}, transparent)` }} />
        </div>
        
        <div className="relative flex items-center h-12 px-3 w-full min-w-0">
          {/* Nebula Logo */}
          <div className="flex items-center gap-2 mr-4 flex-shrink-0">
            <div className="h-8 w-8 rounded-full overflow-hidden shadow-lg ring-2 ring-white/20 flex-shrink-0" style={{ background: `linear-gradient(135deg, ${primaryColor}40, ${secondaryColor}40)` }}>
              <Logo className="h-8 w-8" alt={brandName} />
            </div>
            <span className="text-sm font-bold text-white whitespace-nowrap">{brandName}</span>
          </div>
            ...
            {getFilteredItems(flatMenu).map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 text-slate-300 hover:text-white hover:bg-white/10"
      ...
      {openDropdown && (() => {
        ...
        return (
          <div
            ref={dropdownRef}
            className="fixed bg-slate-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-700 py-2 z-[99999] overflow-hidden"
            style={{ top: dropdownPosition.top, left: dropdownPosition.left, minWidth: '180px' }}
          >
            {/* Nebula glow in dropdown */}
            <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-32 h-32 rounded-full opacity-30 blur-3xl pointer-events-none" style={{ background: colors.base }} />
            
            <div className="relative px-3 py-2 mx-2 rounded-xl mb-1 bg-slate-800/50">
              ...
              {filteredItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium transition-all duration-150 text-left rounded-xl text-slate-300 hover:text-white"
                    style={{ backgroundColor: itemActive ? colors.light + '30' : undefined, color: itemActive ? colors.base : undefined }}
                  >
