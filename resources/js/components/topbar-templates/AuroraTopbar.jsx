// resources/js/components/topbar-templates/AuroraTopbar.jsx
...
export default function AuroraTopbar() {
  ...
  return (
    <>
      <div className="relative bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 overflow-hidden w-full max-w-full">
        {/* Aurora Background Effect */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute -top-20 -left-20 w-60 h-60 rounded-full opacity-20 blur-3xl" style={{ background: `radial-gradient(circle, ${primaryColor}, transparent)` }} />
          <div className="absolute -top-10 right-20 w-40 h-40 rounded-full opacity-15 blur-2xl" style={{ background: `radial-gradient(circle, ${secondaryColor}, transparent)` }} />
        </div>
        
        <div className="relative flex items-center h-12 px-3 w-full min-w-0">
          {/* Aurora Logo */}
          <div className="flex items-center gap-2 mr-4 flex-shrink-0">
            <div className="h-8 w-8 rounded-full overflow-hidden shadow-lg ring-2 flex-shrink-0" style={{ ringColor: primaryColor + '50' }}>
            ...
            {getFilteredItems(flatMenu).map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 hover:bg-gray-100 dark:hover:bg-slate-800"
      ...
      {openDropdown && (() => {
        ...
        return (
          <div
            ref={dropdownRef}
            className="fixed bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-2xl shadow-2xl ring-1 ring-gray-200 dark:ring-slate-700 py-2 z-[99999] overflow-hidden"
            style={{ top: dropdownPosition.top, left: dropdownPosition.left, minWidth: '180px' }}
          >
            {/* Aurora glow in dropdown */}
            <div className="absolute -top-10 -right-10 w-20 h-20 rounded-full opacity-20 blur-2xl pointer-events-none" style={{ background: colors.base }} />
            
            <div className="relative px-3 py-2 mx-2 rounded-xl mb-1" style={{ backgroundColor: colors.light }}>
