...
<head>
<meta charset="utf-8"/>
<title>Quotation (A4) - {{ $posted }}</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .footer-total .row.total { font-weight:900; font-size:14px; }
  ...
  <table>
    ...
    <tbody>
      @foreach($invoice->items as $it)
        <tr>
          <td>{{ $it->line_type === 'manual' ? ($it->manual_name ?? '-') : optional($it->product)->name ?? '-' }}</td>
